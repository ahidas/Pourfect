 ...
 * Writes both the register address and data in single buffer of length two.
 * Compatable with DFRobot_RGBLCD1602.
 *
 ...
 */
unsigned char I2C_WriteRegDFRobot(
    unsigned char I2CAddress,
    unsigned char deviceRegisterAddress,
    uint8_t data
)
{
    ...
    uint8_t buffer[2] = {deviceRegisterAddress, data}; // First byte: Register address, Second byte: Data
